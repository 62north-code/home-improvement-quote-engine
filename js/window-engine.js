(function () {
    ...
    document.querySelectorAll(".glazing-style").forEach((el) => {
        el.addEventListener("click", () => {
            ...
            console.log(
                "Selected glazing style:",
                selectedOptions.glazingStyle
            );
        });
    });
        ...
        switch (stepIndex) {
            ...
            case 3:
                if (!selectedOptions.externalFrameColor) {
                    validationMessage =
                        "Please select an external frame color.";
                }
                break;
            case 4:
                if (!selectedOptions.internalFrameColor) {
                    validationMessage =
                        "Please select an internal frame color.";
                }
                break;
            case 5:
  ...
  `;
    ...
    document.addEventListener("DOMContentLoaded", function () {
        document
            .querySelectorAll(".external-frame-color img")
            .forEach((img) => {
                img.addEventListener("click", (event) => {
                    // Get the window style from the clicked image
    ...
    document.addEventListener("DOMContentLoaded", function () {
        document
            .querySelectorAll(".internal-frame-color img")
            .forEach((img) => {
                img.addEventListener("click", (event) => {
                    // Get the window style from the clicked image
    ...
    function updateSelection(attribute, option) {
        ...
        const attributeImages = document.querySelectorAll(
            `img[data-${attribute}]`
        );
